function crime_map(coordinates, htmlID) {
...
function chi_markers(url, map, year) {
  d3.json(url, function (response) {
    // response = response.filter(.... == year)
    for (var i = 0; i < response.length; i++) {
      var location = response[i];
...
var chiMap = crime_map(coordinates["chicago"], htmlID = "chicago-map")
chi_markers(url["chicago"], chiMap, year = 2018)
